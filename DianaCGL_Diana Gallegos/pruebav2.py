...
def generar_informe_word(calificaciones, promedios_ponderados, calificacion_final, nombre_auditor, nombre_compania, fecha_evaluacion, destinatario, firma):
    document = Document()
...
for aspecto in rubricas.keys():
    ...
    for pregunta in rubricas[aspecto].keys():
        opciones = [f'{i}: {desc}' for i, desc in rubricas[aspecto][pregunta].items()]
        seleccion = st.selectbox(pregunta, opciones, key=f'{aspecto}_{pregunta}')
